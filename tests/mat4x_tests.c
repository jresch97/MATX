...
const VEC4F RIGHT = { 1.0f, 0.0f, 0.0f, 1.0f };
const VEC4F UP    = { 0.0f, 1.0f, 0.0f, 1.0f };
const VEC4F FWD   = { 0.0f, 0.0f, 1.0f, 1.0f };

static inline int vec4feq(const VEC4F a, const VEC4F b)
{
        for (int i = 0; i < 4; i++) if (!FEQ(a[i], b[i])) return 0;
        return 1;
}

static inline void vec4fabs(VEC4F out, const VEC4F v)
{
        for (int i = 0; i < 4; i++) out[i] = fabs(v[i]);
}

static inline void vec4fprintf(const VEC4F v)
{
        printf("%f %f %f %f\n", v[0], v[1], v[2], v[3]);
}
...
int main()
{
        ...
        const float xr = D2R(90.0f), xc = cosf(xr), xs = sinf(xr);
        const float yr = D2R(90.0f), yc = cosf(yr), ys = sinf(yr);
        const float zr = D2R(90.0f), zc = cosf(zr), zs = sinf(zr);
        printf("================================================================================\n");
        printf("= MAT4FID test\n");
        printf("================================================================================\n");
